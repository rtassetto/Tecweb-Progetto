...
Javascript è stato utilizzato per effettuare controlli sugli input degli utenti senza che intervenga il server, infatti tutti i controlli dell'input fatti su php vengono rifatti da javascript e tramite l'evento JS \emph{oninput} viene controllato a tempo reale se l'input è corretto o meno.\newline
Questi controlli permettono di ridurre le chiamate al server.
